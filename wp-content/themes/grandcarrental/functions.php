		...
		<?php 
			foreach ($grandcarrental_options as $value) {
				...
				if($value['type'] == 'section')
				{
					...
					echo '<a id="pp_panel_'.esc_attr(strtolower($value['name'])).'_a" href="#pp_panel_'.esc_attr(strtolower($value['name'])).'" class="nav-tab '.esc_attr($active).'"><i class="fa '.esc_attr($value['icon']).'"></i>'.str_replace('-', ' ', esc_html($value['name'])).'</a>';
				}
			}
		?>
		</h2>
...
function grandcarrental_import_demo_content() {
	...
	if(is_admin() && isset($_POST['demo']) && !empty($_POST['demo']))
	{
		...
		$styling_file = get_template_directory() . "/cache/demos/1.dat";
