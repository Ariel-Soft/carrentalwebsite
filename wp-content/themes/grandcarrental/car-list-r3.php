...
<div class="inner">
	...
	<!-- Nav tabs -->
		<ul class="nav nav-tabs" role="tablist">
			...
			<li role="presentation" class="active" ><a href="" aria-controls="messages" role="tab" data-toggle="tab">3 Rental Option</a></li>
		...
		<div class="tab-content">
			<div role="tabpanel" class="tab-pane <?php if($_GET['vehtype']!= 0) { ?> active <?php } ?>" id="messages">	
				...
				if($_GET['VehicleType']!= 0) {
					...
					$curl_post_dataveh2  = "{\"ClientId\":\"6189\",\"UserEmail\":\"ahmed.fattoh2@rentcentric.com\",\"Password\":\"Rent45\",\"PickupDate\":\"7/27/2017\",\"PickupLocation\":\"$picklocationname\",\"PickupTime\":\"12:00 PM\",\"DropOffDate\":\"7/27/2017\",\"DropOffLocation\":\"Sherbrooke \",\"DropOffTime\":\"12:00 PM\"}";
					curl_setopt($ch, CURLOPT_HTTPHEADER, array("cache-control: no-cache","content-type: application/json; charset=utf-8"));
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_POST, true);
					curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
					...
					$curl_post_datarate2  = "{\"ClientId\":\"6189\",\"UserEmail\":\"ahmed.fattoh2@rentcentric.com\",\"Password\":\"Rent45\",\"PickupDate\":\"7/17/2017\",\"LocationID\":\"116\",\"PickupLocation\":\"117\",\"PickupTime\":\"12:00 PM\",\"DropOffDate\":\"7/27/2017\",\"DropOffLocation\":\"122\",\"DropOffTime\":\"12:00 PM\"}";
					curl_setopt($ch, CURLOPT_HTTPHEADER, array("cache-control: no-cache","content-type: application/json; charset=utf-8"));
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_POST, true);
					curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
						...
						<table class="table">
							<tbody>
								...
								<tr>
									...
									<td><h4><span>$ </span><span id="taxsc1"><?php if(!empty($_GET['taxcharges'])){ echo $_GET['taxcharges'];}else{echo"0:00";} ?></span></h4></td>
								</tr>
								<tr>
									<td><h4>Total</h4></td>
				...
				?>
					...
					<div class="service_div border_top service_div2">
						...
						<span>Make your drive come alive with SiriusXM<b>&reg;</b> Satellite Radio.</span>
						<form class="form_btn">
							<label><input type="checkbox" name="sateliteradio" value="<?php if(isset($_GET['days'])){echo $_GET['days']*6.99;}else{echo "6.99";} ?>"> $6.99/Day</label>
							<div class="clearfix"></div>
						</form>
					</div>
					<div class="service_div border_top safety_div">
						...
						<span>Pease of Mind for unexpected emergencies. Get fast, reliable 24/7 service when you need it most.</span>
						<form class="form_btn">
							<label><input type="checkbox" name="exroadside" value="<?php if(isset($_GET['days'])){echo $_GET['days']*16.99;}else{echo"16.99";} ?>"> $16.99/Day</label>
							<div class="clearfix"></div>
						</form>
					</div>
					<div class="service_div border_top safety_div">
						...
						<span>Keep kids safe when travelling with our child safety seats.</span>
						<form class="form_btn">
							<label><input type="checkbox" name="childsafety" value="<?php if(isset($_GET['days'])){echo $_GET['days']*6.99;}else{ echo"6.99"; } ?>"> 6.99/Day</label>
							<div class="clearfix"></div>
						</form>
					</div>
					<div class="service_div border_top safety_div">
...
jQuery(document).ready(function () {
	...
	/*jQuery("#continue_btnBtn").on('click', function() {
			jQuery("input[name='sateliteradio']").prop('checked', false);
			jQuery("input[name='exroadside']").prop('checked', false);
			jQuery("input[name='childsafety']").prop('checked', false);
	});*/
	...
	jQuery(window).load(function() {
		...
		var tstval3=0;
		if (ckbox.is(':checked')) {
			tstval=jQuery("input[name='sateliteradio']").val();
		}
		if (ckbox1.is(':checked')) {
			tstval2=jQuery("input[name='exroadside']").val();
		}
		if (ckbox2.is(':checked')) {
			tstval3=jQuery("input[name='childsafety']").val();
		}
		var tsttotal=Number(tstval)+Number(tstval2)+Number(tstval3);
		var tstotal=tsttotal.toFixed(2);
		jQuery("#rentaloption1").text(tstotal);
			jQuery("input[name='rentalopt']").val(tstotal);
	});
