...
include_once('Backand.php');
$backandObj = new Backand();

if(isset($_GET['auth_id']) && isset($_GET['auth_app']) && isset($_GET['force'])){
    $backandObj->authenticateSession(filter_input(INPUT_GET,'auth_id'), filter_input(INPUT_GET,'auth_app'), filter_input(INPUT_GET,'force'), $configData);
}
...
$subId = $backandObj->getSessionSubscriptionId();

if(!isset($subId)){
    exit('No billing info for this app');
}

$servicePortal = new ServicePortal($subId);
